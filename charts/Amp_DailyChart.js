...
const Amp_DailyChart = () => {
  const [tooltip, setTooltip] = useState({ visible: false, xValue: '', yValue: '', x: 0, y: 0 });
  const screenWidth = Dimensions.get('window').width;
  const voltage = 230; // Common household voltage in many countries
  ...
  // Convert kWh data to amperes assuming each kWh value represents average power over an hour
  const chartData = {
    labels: ['12 AM', "", "", "", '4 AM', "", "", "", '8 AM', "", "", "", '12 PM'],
    datasets: [{
      data: [5, 4.5, 4, 5.5, 5, 3, 2, 1.5, 0.5, 1, 2.5, 1.5, 1.5].map(kWh => (kWh * 1000) / voltage) // Convert kWh to Amps
    }]
  };
  ...
  const handleDataPointClick = (data, index) => {
    ...
    setTooltip({
      ...
      yValue: value.toFixed(2), // Displaying in Amperes
      x: xPos,
      y: yPos
    });
  };
          ...
          if (tooltip.visible) {
            return (
              <View style={styles.tooltipStyle(tooltip.x, tooltip.y)}>
                ...
                <Text style={styles.tooltipText}>{`الاستهلاك: ${tooltip.yValue} A`}</Text>
              </View>
            );
          }
        }}
