...
const Amp_RealTimeChart = ({ apiUrl }) => {
        ...
        for (const deviceId in json) {
          ...
          if (device && typeof device === 'object' && device.channels) {
            for (const channelId in device.channels) {
              ...
              if (channel && typeof channel === 'object' && !isNaN(channel.amperes)) {
                totalAmperes += Number(channel.amperes);
              }
            }
          }
        }
...
export default Amp_RealTimeChart;